...
options['host_file']     = './host-lists.txt'# -R, --host_file <ROUTER_FILE>
...
def getfinal(host, options):  # {{{1}}}
    ...
    if options.get('host', {}):
        ...
        if not final.get('access', None):
            for key in final['access']:
                if not final['access'].get(key, None):
...
def args_def(options): # {{{1}}}
    ...
    parser.add_argument('-n', 
                        dest='processes',
                        help='(TODO) number of concurrent processes')
    parser.add_argument('-P', action='store',
                        dest='protocol',
    ...
    group.add_argument('-d', action='count',
                        default=0,
                        dest='debug',
